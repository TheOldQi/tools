 ...
 * @since java 1.7.0
 */
class QrCodeResolveHelper extends LuminanceSource {

    private final BufferedImage image;
    private final int left;
    private final int top;

    QrCodeResolveHelper(BufferedImage image) {
        this(image, 0, 0, image.getWidth(), image.getHeight());
    }

    QrCodeResolveHelper(BufferedImage image, int left, int top, int width, int height) {
        super(width, height);
