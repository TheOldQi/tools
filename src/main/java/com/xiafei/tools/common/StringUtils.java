...
public final class StringUtils extends org.apache.commons.lang3.StringUtils {
    ...
    /**
     * 判断两个日志是否同类.
     * 同类是指同一个记录日志的位置，判断依据如下：
     * 1、loggerName相同
     * 2、日志正文如果从汉字开始，判断两条日志连续汉字是否一致，完全一致判断为同类
     * 3、日志正文如果从英文字母开始，判断两条日志连续英文字母或空格、换行符是否一致且超过50个字符或到达终点
     * ，若一致字母数未达到50个，但大于10且前一个字符是=、:、{、[，如果是，可以认为同类，否则不同类
     *
     * @param logBody1 第一个日志正文
     * @param logBody2 第二个日志正文
     * @return true-同类，false-不同类
     */
    public static boolean similarLog(final String logBody1, final String logBody2) {
        if (isBlank(logBody1) || isBlank(logBody2)) {
            return false;
        }
        int lengthA = logBody1.length();
        int lengthB = logBody2.length();

        boolean startWithChinese = false;
        for (int i = 0, j = 0, cc /* 比较次数 */ = 1, ec /* 相等计数 */ = 0; (i < lengthA) && (j < lengthB); i++, j++, cc++) {
            char a = ' ';
            char b = ' ';
            for (; i < lengthA; i++) {
                if (!Character.isWhitespace(a = logBody1.charAt(i))) {
                    break;
                }
            ...
            for (; j < lengthB; j++) {
                if (!Character.isWhitespace(b = logBody2.charAt(j))) {
                    break;
                }
            }
            ...
            if (startWithChinese) {
                ...
                if (a == b) {
                    ...
                    if (ec > 10) {
                        // 上一次一定是相等的才能到这，取一个就行了
                        final char c = logBody1.charAt(i - 1);
                        if (c == '=' || c == '[' || c == ':' || c == '{') {
                            return true;
                        }
                    } else {
