...
public class Sftp {
     ...
     * 调用时间池.
     */
    private final LinkedList<Long> timePool = new LinkedList<>();
    ...
    public Sftp(final String host, final Integer port, final String userName, final String password,
                final Integer timeOut, final Integer initSize, final Integer maxSize, final Integer heartBeatInterval) {
        this.host = host;
    ...
    public byte[] getBytes(final String path) {
        ...
        try {
            ...
            try (InputStream is = channel.get(path);
                ...
                final byte[] buffer = new byte[CommonConst.SFTP_BUFFER_SIZE];
                int remain;
                while ((remain = is.read(buffer, 0, CommonConst.SFTP_BUFFER_SIZE)) > 0) {
                    bos.write(buffer, 0, remain);
                }
                return bos.toByteArray();
            } catch (SftpException e) {
                log.error("{}sftp操作异常，获取path={}文件的输出流失败", logPrefix, path, e);
                throw new RuntimeException(e);
            } catch (IOException e) {
                log.error("{}输出流关闭失败", logPrefix, e);
                throw new RuntimeException(e);
            }
    ...
    public void removeSync(final String path, final boolean isDirectory, final String serialNo) {
        ...
        try {
            ...
            if (path.contains(CommonConst.FILE_SEPARATOR_FORWARD)) {
                final int lastIndex = path.lastIndexOf(CommonConst.FILE_SEPARATOR_FORWARD);
                directory = path.substring(0, lastIndex);
                fileName = path.substring(lastIndex + 1);
            } else if (path.contains(CommonConst.FILE_SEPARATOR_BACK)) {
                final int lastIndex = path.lastIndexOf(CommonConst.FILE_SEPARATOR_BACK);
                directory = path.substring(0, lastIndex);
                fileName = path.substring(lastIndex + 1);
            } else {
                directory = null;
    ...
    private void keepAlive() {
        ...
        for (int i = 0; i < maxSize; i++) {
            if (!timePool.isEmpty() && (l - timePool.getFirst() > heartBeatInterval)) {
                try {
                    // 每次执行fetch都会弹出pool和timePool的一个元素，
                    // 下次循环进来继续拿最早归还连接池的连接时间判断是否
                    // 已经超过心跳间隔，如果超过就执行这段代码，如此循环，
                    // 知道连接池空了或者达到连接池最大长度
                    release(fetch());
                } catch (Exception e) {
                    log.error("{}心跳一个连接失败", logPrefix, e);
                }
                log.info("{}心跳完成+1", logPrefix);
            } else {
                return;
            }
        }
    ...
    private void cycleMkDir(final String path, final ChannelSftp channelSftp) {
        if (path.contains(CommonConst.FILE_SEPARATOR_FORWARD)) {
            cycleMkDir(path, channelSftp, CommonConst.FILE_SEPARATOR_FORWARD);
        } else if (path.contains(CommonConst.FILE_SEPARATOR_BACK)) {
            cycleMkDir(path, channelSftp, CommonConst.FILE_SEPARATOR_BACK);
        }
        ...
        if (channel != null) {
            synchronized (pool) {
                if (!pool.isEmpty() && poolSize > initSize && needSize.get() < pool.size()) {
                    ...
                    timePool.addLast(System.currentTimeMillis());
                }
                pool.notify();
